...
subplot(1,2,1),    plot(t,pt(t)/1e3,'-b')
xlabel('t [sec]'), ylabel('presion [kPa]')
axis([t0,0.3*max(t),1.2*min(min(pt(t)/1e3)),1.2*max(max(pt(t)/1e3))])
title('Presion por Explosion')

subplot(1,2,2),    plot(t,u(1,:),'-b',t,u(2,:),'-r',t,u(3,:),'-k')
legend('u1(t)','u2(t)','u3(t)'), xlabel('t [sec]'), ylabel('desplazamiento [m]')
