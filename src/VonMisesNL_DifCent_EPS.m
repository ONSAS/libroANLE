...
subplot(3,1,1)
plot(t(1:10:end),1000*u(1,1:10:end), 'linewidth', 2)
labx=xlabel('$t$ [s]'); laby=ylabel('$u_1$ [mm]');
axis([0 2 1e3*min(u(1,:))*1.1 1e3*max(u(1,:))*1.1]);

set(labx, "FontSize", 16); set(laby, "FontSize", 16);

set(gca, 'fontsize', 16 )

subplot(3,1,2)
plot(t(1:10:end),1000*u(2,1:10:end), 'linewidth', 2)
labx=xlabel('$t$ [s]'); laby=ylabel('$u_2$ [mm]');
...
subplot(3,1,3)
plot(t(1:10:end),E*Ac*epsg(1:10:end), 'linewidth', 2)
labx=xlabel('$t$ [s]'); laby=ylabel('Directa [N]')
